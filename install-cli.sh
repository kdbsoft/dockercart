...
until docker compose exec -T -e MYSQL_PWD mariadb mariadb-admin ping -h 127.0.0.1 -u "${MARIADB_USER:-dockercart}" --silent >/dev/null 2>&1; do
    echo -n "."
    sleep 2
done
echo -e " ${GREEN}✓${NC}"

TABLE_COUNT=$(docker compose exec -T -e MYSQL_PWD mariadb mariadb -N -B -u "${MARIADB_USER:-dockercart}" "${MARIADB_DATABASE:-${DB_DATABASE}}" -e "SHOW TABLES" 2>/dev/null | wc -l | tr -d ' ')
