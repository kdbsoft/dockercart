...
<?php
/**
 * DockerCart Search — Manticore Reindex CLI Worker
 *
 * Rebuilds all Manticore RT indexes (products, categories, manufacturers,
 * information, orders, customers). Called by the scheduler daemon (daily)
 * so indexes survive Manticore container restarts (tmpfs data dir).
 *
 * Usage:
...
try {
	...
	fwrite(STDOUT, "[dockercart-reindex] Starting Manticore reindex...\n");
	...
	fwrite(STDERR, "[dockercart-reindex] FATAL: " . $e->getMessage() . "\n");
	exit(1);
}
