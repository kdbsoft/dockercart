...
class ControllerExtensionModuleDockercartMulticurrency extends Controller
{
    ...
    public function index()
    {
        ...
        if (
            ...
            $this->model_setting_setting->editSetting(
                "module_dockercart_multicurrency",
                $this->request->post,
            );
        ...
        $data["breadcrumbs"][] = [
            ...
            "href" => $this->url->link(
                "extension/module/dockercart_multicurrency",
                "user_token=" . $this->session->data["user_token"],
                true,
            ),
        ];
        ...
        if (
            isset(
                $this->request->post["module_dockercart_multicurrency_status"],
            )
        ) {
            $data["module_dockercart_multicurrency_status"] =
                $this->request->post["module_dockercart_multicurrency_status"];
        } else {
            $data[
                "module_dockercart_multicurrency_status"
            ] = $this->config->get("module_dockercart_multicurrency_status");
        }
        ...
        if (
            ...
            $data["module_dockercart_multicurrency_license_key"] =
                $this->request->post[
                    "module_dockercart_multicurrency_license_key"
                ];
        } else {
        ...
        if (
            ...
            $data["module_dockercart_multicurrency_public_key"] =
                $this->request->post[
                    "module_dockercart_multicurrency_public_key"
                ];
        } else {
    ...
    public function install()
    {
        ...
        $query = $this->db->query(
            "SHOW COLUMNS FROM `" . DB_PREFIX . "product` LIKE 'currency_id'",
        );
        ...
        if (!$query->num_rows) {
            $this->db->query(
                "ALTER TABLE `" .
                    DB_PREFIX .
                    "product` ADD `currency_id` INT(11) DEFAULT NULL AFTER `price`",
            );
        }
        ...
        $events = [
            ...
            [
                ...
                "action" =>
                    "extension/module/dockercart_multicurrency/eventProductFormBefore",
            ],
            ...
            [
                ...
                "action" =>
                    "extension/module/dockercart_multicurrency/eventProductFormAfter",
            ],
            ...
            [
                ...
                "action" =>
                    "extension/module/dockercart_multicurrency/eventProductAddAfter",
            ],
            ...
            [
                ...
                "action" =>
                    "extension/module/dockercart_multicurrency/eventProductEditBefore",
            ],
            ...
            [
                ...
                "action" =>
                    "extension/module/dockercart_multicurrency/eventProductListBefore",
            ],
            ...
            [
                ...
                "action" =>
                    "extension/module/dockercart_multicurrency/eventModelProductBefore",
                "sort_order" => 0,
            ],
            ...
            [
                ...
                "action" =>
                    "extension/module/dockercart_multicurrency/eventModelProductAfter",
                "sort_order" => 999,
            ],
    ...
    public function uninstall()
    {
        ...
        $this->db->query(
            "DELETE FROM `" .
                DB_PREFIX .
                "event` WHERE `code` LIKE 'dockercart_multicurrency_%'",
        );
    ...
    private function installOCMOD()
    {
        ...
        $modification_query = $this->db->query(
            "SELECT modification_id FROM " .
                DB_PREFIX .
                "modification WHERE code = 'dockercart_multicurrency_cart'",
        );

        if ($modification_query->num_rows) {
            // Remove old version
            $this->model_setting_modification->deleteModification(
                $modification_query->row["modification_id"],
            );
        }
    ...
    public function eventProductFormBefore(&$route, &$data, &$template)
    {
        ...
        // Load language for product form labels
        $this->load->language("extension/module/dockercart_multicurrency");
    ...
    public function eventProductFormAfter(&$route, &$data, &$output)
    {
        ...
        $currency_html .=
            ...
            $this->language->get("help_product_currency") .
            '" data-original-title="' .
            $this->language->get("entry_product_currency") .
            '">' .
            $this->language->get("entry_product_currency") .
            "</span>";
        $currency_html .= "</label>" . "\n";
        ...
        $currency_html .=
            ">" .
            $this->language->get("text_use_default_currency") .
            "</option>" .
            "\n";
        ...
        foreach ($data["currencies"] as $currency) {
            $currency_html .=
                '                <option value="' .
                (int) $currency["currency_id"] .
                '"';
            if (
                isset($data["product_currency_id"]) &&
                $currency["currency_id"] == $data["product_currency_id"]
            ) {
                $currency_html .= ' selected="selected"';
            }
            $currency_html .=
                ">" .
                htmlspecialchars($currency["title"], ENT_QUOTES, "UTF-8") .
                " (" .
                htmlspecialchars($currency["code"], ENT_QUOTES, "UTF-8") .
                ")</option>" .
                "\n";
        }
    ...
    public function eventProductAddBefore(&$route, &$args)
    {
        ...
        if (
            isset($this->request->post["currency_id"]) &&
            (int) $this->request->post["currency_id"] > 0
        ) {
        ...
        if (
            ...
            if ($product_id) {
                $this->db->query(
                    "UPDATE " .
                        DB_PREFIX .
                        "product SET currency_id = '" .
                        (int) $this->request->post["currency_id"] .
                        "' WHERE product_id = '" .
                        (int) $product_id .
                        "'",
                );
        ...
        if (isset($data["products"])) {
            ...
            foreach ($data["products"] as &$product) {
                ...
                if ($query->num_rows && $query->row["currency_id"]) {
                    $currency = $this->model_localisation_currency->getCurrency(
                        $query->row["currency_id"],
                    );
