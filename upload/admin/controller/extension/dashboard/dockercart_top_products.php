...
class ControllerExtensionDashboardDockercartTopProducts extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('dashboard_dockercart_top_products', $this->request->post);
		...
		$data['action'] = $this->url->link('extension/dashboard/dockercart_top_products', 'user_token=' . $this->session->data['user_token'], true);

		$data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=dashboard', true);

		if (isset($this->request->post['dashboard_dockercart_top_products_width'])) {
			$data['dashboard_dockercart_top_products_width'] = $this->request->post['dashboard_dockercart_top_products_width'];
		} else {
			$data['dashboard_dockercart_top_products_width'] = $this->config->get('dashboard_dockercart_top_products_width');
		}
		...
		if (isset($this->request->post['dashboard_dockercart_top_products_status'])) {
			$data['dashboard_dockercart_top_products_status'] = $this->request->post['dashboard_dockercart_top_products_status'];
		} else {
			$data['dashboard_dockercart_top_products_status'] = $this->config->get('dashboard_dockercart_top_products_status');
		}

		if (isset($this->request->post['dashboard_dockercart_top_products_sort_order'])) {
			$data['dashboard_dockercart_top_products_sort_order'] = $this->request->post['dashboard_dockercart_top_products_sort_order'];
		} else {
			$data['dashboard_dockercart_top_products_sort_order'] = $this->config->get('dashboard_dockercart_top_products_sort_order');
		}
		...
		$this->response->setOutput($this->load->view('extension/dashboard/dockercart_top_products_form', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/dashboard/dockercart_top_products')) {
