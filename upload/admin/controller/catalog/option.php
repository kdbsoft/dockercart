...
class ControllerCatalogOption extends Controller {
		...
		if (!isset($json['error'])) {
			...
			if ($field === 'sort_order') {
				...
				if ($val < 0) {
					...
					$this->model_catalog_option->updateOptionField($option_id, array('sort_order' => $val));
					$json['success'] = true;
	...
	public function getName() {
		...
		if (!isset($json['error'])) {
			...
			$descriptions = $this->model_catalog_option->getOptionDescriptions($option_id);
	...
	public function updateNames() {
		...
		if (!isset($this->request->post['option_id']) || !isset($this->request->post['names'])) {
			$json['error'] = 'Invalid request';
		}
		...
		if (!isset($json['error'])) {
			...
			if (!empty($error_names)) {
				...
				$this->model_catalog_option->updateOptionNames($option_id, $names);
				$json['success'] = true;
