...
class ControllerCatalogManufacturer extends Controller {
	...
	protected function getForm() {
		...
		$data['text_form_subtitle'] = !isset($this->request->get['manufacturer_id'])
		    ? $this->language->get('text_add_manufacturer_subtitle')
		    : $this->language->get('text_edit_manufacturer_subtitle');
		...
		if (isset($this->request->post['image']) && is_file(DIR_IMAGE . $this->request->post['image'])) {
			...
			$data['thumb'] = $this->model_tool_image->resize($manufacturer_info['image'], 140, 140);
		} else {
	...
	protected function validateForm() {
		...
		// Validate manufacturer descriptions
		if (isset($this->request->post['manufacturer_description'])) {
	...
	public function updateField() {
		...
		if (!isset($this->request->post['manufacturer_id']) || !isset($this->request->post['field']) || !isset($this->request->post['value'])) {
		...
		if (!isset($json['error'])) {
			...
			if ($field === 'sort_order') {
				...
				if ($val < 0) {
					...
					$this->model_catalog_manufacturer->updateManufacturerField($manufacturer_id, array('sort_order' => $val));
					$json['success'] = true;
	...
	public function getName() {
		...
		if (!isset($json['error'])) {
			...
			$descriptions = $this->model_catalog_manufacturer->getManufacturerDescriptions($manufacturer_id);
	...
	public function updateNames() {
		...
		if (!isset($this->request->post['manufacturer_id']) || !isset($this->request->post['names'])) {
			$json['error'] = 'Invalid request';
		}
		...
		if (!isset($json['error'])) {
			...
			if (!empty($error_names)) {
				...
				$this->model_catalog_manufacturer->updateManufacturerNames($manufacturer_id, $names, $this->config->get('config_language_id'));
				$json['success'] = true;
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name'])) {
			...
			$manticore = $this->model_common_admin_search->searchEntity('manufacturer', $filter_name, array('limit' => 5));
