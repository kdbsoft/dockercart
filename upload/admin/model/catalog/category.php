...
class ModelCatalogCategory extends Model {
	...
	public function editCategory($category_id, $data) {
		...
		$query = $this->db->query("SELECT DISTINCT language_id FROM " . DB_PREFIX . "language");
		$languages = $query->rows;
		$query = $this->db->query("SELECT DISTINCT store_id FROM " . DB_PREFIX . "store");
		$stores = $query->rows;
		
		foreach ($languages as $lang) {
			foreach ($stores as $store) {
				$this->cache->delete('category.' . (int)$category_id . '.' . (int)$lang['language_id'] . '.' . (int)$store['store_id']);
			}
		}
	}
	...
	public function copyCategory($category_id)
	{
		...
		$new_category_id = $this->addCategory($data);

		// Recursively copy child categories
		$children_query = $this->db->query(
			"SELECT category_id FROM " .
				DB_PREFIX .
				"category WHERE parent_id = '" .
				(int) $category_id .
				"'",
		);

		foreach ($children_query->rows as $child) {
			$this->copyChildCategory(
				(int) $child["category_id"],
				$new_category_id,
			);
		}
	...
	private function copyChildCategory($category_id, $new_parent_id)
	{
		$query = $this->db->query(
			"SELECT * FROM " .
				DB_PREFIX .
		...
		$new_child_id = $this->addCategory($data);
	...
	public function updateCategoryNames($category_id, $names) {
		foreach ($names as $language_id => $name) {
			...
			$this->db->query("UPDATE " . DB_PREFIX . "category_description SET name = '" . $this->db->escape($name) . "' WHERE category_id = '" . (int)$category_id . "' AND language_id = '" . (int)$language_id . "'");
		}
	}
}