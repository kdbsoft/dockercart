...
class ModelWarehouseStock extends Model {
	...
	public function getProductStocks(int $product_id, int $variant_id = 0): array {
		$query = $this->db->query("SELECT ws.*, w.name AS warehouse_name FROM `" . DB_PREFIX . "warehouse_stock` ws LEFT JOIN `" . DB_PREFIX . "warehouse` w ON (w.warehouse_id = ws.warehouse_id) WHERE ws.`product_id` = '" . (int)$product_id . "' AND ws.`variant_id` = '" . (int)$variant_id . "'");
	...
	public function getStockMatrix(array $data = []): array {
		...
		$sql = "SELECT ws.*, p.model AS product_model, pd.`name` AS product_name, pv.sku AS variant_sku, w.`name` AS warehouse_name, w.`type` AS warehouse_type
