<?php

$_['column_action'] = 'Действие';
$_['column_name'] = 'Название опции';
$_['column_sort_order'] = 'Порядок сортировки';
$_['column_status'] = 'Статус';
$_['entry_color'] = 'Цвет';
$_['entry_name'] = 'Название опции';
$_['entry_option_value'] = 'Название значения параметра';
$_['entry_sort_order'] = 'Порядок сортировки';
$_['entry_status'] = 'Статус';
$_['entry_show_option_price'] = 'Показывать цену возле значений опции';
$_['entry_type'] = 'Тип';
$_['error_name'] = 'Название параметра должно содержать от 1 до 128 символов!';
$_['error_option_value'] = 'Имя параметра должно содержать от 1 до 128 символов!';
$_['error_permission'] = 'Внимание: у вас нет разрешения на изменение параметров!';
$_['error_product'] = 'Внимание: Этот параметр нельзя удалить, так как в данный момент он назначен %s товарам!';
$_['error_invalid_sort_order'] = 'Неверное значение порядка сортировки!';
$_['error_type'] = 'Внимание: Требуются значения параметров!';
$_['heading_title'] = 'Опции';
$_['text_add'] = 'Добавить опцию';
$_['text_checkbox'] = 'Флажок';
$_['text_color'] = 'Цвет';
$_['text_choose'] = 'Выбирать';
$_['text_date'] = 'Дата';
$_['text_datetime'] = 'Дата и время';
$_['text_edit'] = 'Редактирование опции';
$_['text_file'] = 'Файл';
$_['text_input'] = 'Вход';
$_['text_list'] = 'Список опций';
$_['text_option'] = 'Опция';
$_['text_radio'] = 'Радио';
$_['text_select'] = 'Выбирать';
$_['text_success'] = 'Успех: Вы изменили опции!';
$_['text_text'] = 'Текст';
$_['text_textarea'] = 'Текстовое поле';
$_['text_time'] = 'Время';
$_['text_value'] = 'Значения опций';
// Sidebar
$_['text_settings_card']     = 'Параметры';
$_['text_status_card']       = 'Статус';

// Subtitle

$_['text_list_subtitle'] = 'Управляйте опциями и значениями товаров';

$_['text_add_option_subtitle'] = 'Определить новую опцию товара';

$_['text_edit_option_subtitle'] = 'Редактировать значения и настройки опции';

// Empty state
$_['text_empty_title']       = 'Пока нет опций';
$_['text_empty_description'] = 'Опции позволяют выбирать варианты — размер, материал и т.п. Добавьте первую опцию.';
