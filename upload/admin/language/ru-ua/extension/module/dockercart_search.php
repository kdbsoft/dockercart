<?php

$_['button_cancel'] = 'Отмена';
$_['button_reindex'] = 'Переиндексировать всё';
$_['button_save'] = 'Сохранить';
$_['button_test_connection'] = 'Проверить соединение';
$_['button_add_mapping'] = 'Добавить соответствие';
$_['button_export_csv'] = 'Выгрузить CSV';
$_['button_import_csv'] = 'Загрузить CSV';
$_['button_manage_mappings'] = 'Управлять соответствиями';
$_['entry_autocomplete'] = 'Включить автодополнение';
$_['entry_voice_search'] = 'Включить голосовой поиск';
$_['entry_autocomplete_limit'] = 'Лимит автодополнения';
$_['entry_field_weights'] = 'Веса полей';
$_['entry_host'] = 'Хост Manticore';
$_['entry_http_port'] = 'Порт HTTP API';
$_['entry_min_chars'] = 'Минимум символов';
$_['entry_morphology'] = 'Морфология';
$_['entry_port'] = 'Порт MySQL протокола';
$_['entry_query_mappings'] = 'Соответствия поисковых запросов';
$_['entry_source'] = 'Исходный запрос';
$_['entry_target'] = 'Заменяющий запрос';
$_['column_source'] = 'Источник';
$_['column_target'] = 'Цель';
$_['column_action'] = 'Действие';
$_['entry_ranking'] = 'Режим ранжирования';
$_['entry_results_limit'] = 'Лимит результатов поиска';
$_['entry_status'] = 'Статус';
$_['entry_weight_description'] = 'Вес описания';
$_['entry_weight_meta'] = 'Вес мета-данных';
$_['entry_weight_tags'] = 'Вес тегов';
$_['entry_weight_title'] = 'Вес названия';
$_['error_autocomplete_limit'] = 'Лимит автодополнения должен быть числом!';
$_['error_host'] = 'Хост обязателен!';
$_['error_min_chars'] = 'Минимум символов должен быть числом!';
$_['error_permission'] = 'Предупреждение: У вас нет прав для изменения этого модуля!';
$_['error_port'] = 'Порт должен быть числом!';
$_['error_results_limit'] = 'Лимит результатов должен быть числом!';
$_['error_source'] = 'Источник обязателен и не должен содержать "=" или перенос строки!';
$_['error_target'] = 'Цель обязательна и не должна содержать перенос строки!';
$_['error_upload'] = 'Пожалуйста, выберите CSV-файл для загрузки!';
$_['heading_title'] = 'Поиск';
$_['heading_title_menu'] = 'Поисковый индекс';
$_['heading_mappings'] = 'Соответствия поисковых запросов';
$_['heading_mapping'] = 'Соответствие поискового запроса';
$_['help_autocomplete'] = 'Включить AJAX автодополнение в поле поиска';
$_['help_voice_search'] = 'Добавляет кнопку микрофона в поле поиска (Web Speech API). Работает в Chrome, Edge и Safari по HTTPS; в неподдерживаемых браузерах кнопка скрывается автоматически. Распознанная фраза заполняет поле без отправки формы.';
$_['help_autocomplete_limit'] = 'Количество подсказок в выпадающем списке';
$_['help_field_weights'] = 'Важность каждого поля при поиске (больше = важнее)';
$_['help_host'] = 'Hostname Manticore Search (по умолчанию: manticore)';
$_['help_http_port'] = 'Порт HTTP API для автодополнения (по умолчанию: 9308)';
$_['help_min_chars'] = 'Минимальное количество символов для поиска/автодополнения';
$_['help_morphology'] = 'Выберите ОДИН морфологический процессор для этого языка (стемминг или лемматизация). После изменения необходимо пересоздать индексы!';
$_['help_port'] = 'Порт MySQL протокола (по умолчанию: 9306)';
$_['help_query_mappings'] = 'Соответствия заменяют один поисковый запрос другим перед поиском. Управляется на отдельной странице.';
$_['help_source'] = 'Запрос, как его вводят покупатели (без учёта регистра). Не может содержать "=" или перенос строки.';
$_['help_target'] = 'Запрос, на который он будет заменён перед поиском.';
$_['help_import_csv'] = 'CSV-файл с двумя колонками: источник,цель. Первая строка может быть заголовком. Существующие источники будут заменены.';
$_['help_query_mappings_page'] = 'Добавление, редактирование, удаление, импорт и экспорт соответствий.';
$_['help_ranking'] = 'Алгоритм ранжирования результатов поиска';
$_['help_reindex'] = 'Перестроить поисковый индекс для всех товаров, категорий, производителей и информационных страниц';
$_['help_results_limit'] = 'Количество результатов поиска на странице по умолчанию';
$_['help_status'] = 'Включить или выключить поиск Manticore';
$_['tab_about'] = 'О модуле';
$_['tab_autocomplete'] = 'Автодополнение';
$_['tab_connection'] = 'Настройки подключения';
$_['tab_general'] = 'Основные настройки';
$_['tab_indexing'] = 'Индексация';
$_['tab_morphology'] = 'Язык и морфология';
$_['text_connection_failed'] = 'Не удалось подключиться к Manticore Search!';
$_['text_connection_success'] = 'Успешно подключено к Manticore Search!';
$_['text_contact'] = 'Контакт';
$_['text_developer'] = 'Разработчик';
$_['text_developer_name'] = 'DockerCart Official';
$_['text_disabled'] = 'Выключено';
$_['text_edit'] = 'Редактирование настроек поиска';
$_['text_enabled'] = 'Включено';
$_['text_extension'] = 'Дополнения';
$_['text_module_settings'] = 'Настройки модуля';
$_['text_no'] = 'Нет';
$_['text_reindex_failed'] = 'Индексация не удалась!';
$_['text_reindex_success'] = 'Индексация завершена: %s товаров, %s категорий, %s производителей, %s информационных страниц';
$_['text_success'] = 'Успех: Настройки модуля сохранены!';
$_['text_tab_about_subtitle'] = 'Информация о модуле и контакты службы поддержки';
$_['text_tab_autocomplete_subtitle'] = 'Ограничения на поведение автозаполнения и предлагаемые варианты.';
$_['text_tab_connection_subtitle'] = 'Конфигурация хоста и портов Manticore';
$_['text_tab_general_subtitle'] = 'Основные параметры поиска и ограничения на количество результатов.';
$_['text_tab_indexing_subtitle'] = 'Переиндексировать данные каталога в Manticore';
$_['text_yes'] = 'Да';
$_['text_reindex_warning'] = 'Переиндексация может занять несколько минут для больших каталогов (>10 000 товаров).';
$_['text_reindexing'] = 'Переиндексация...';
$_['text_reindex_confirm'] = 'Вы уверены, что хотите переиндексировать все данные? Это может занять несколько минут.';
$_['text_error_label'] = 'Ошибка';
$_['text_success_delete'] = 'Успех: Соответствие удалено!';
$_['text_success_mapping'] = 'Успех: Соответствие сохранено!';
$_['text_confirm_delete'] = 'Удалить это соответствие?';
$_['text_import_success'] = 'Импорт завершен: импортировано соответствий: %s.';
$_['text_import_skipped'] = 'Пропущено некорректных строк: %s.';

$_['text_reindex_success'] = 'Индексация завершена: %s товаров, %s категорий, %s производителей, %s информационных страниц';
$_['text_reindex_failed'] = 'Индексация не удалась!';
