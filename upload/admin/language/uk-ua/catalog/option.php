<?php

$_['column_action'] = 'Дія';
$_['column_name'] = 'Назва опції';
$_['column_sort_order'] = 'Порядок сортування';
$_['column_status'] = 'Статус';
$_['entry_color'] = 'Колір';
$_['entry_name'] = 'Назва опції';
$_['entry_option_value'] = 'Назва значення опції';
$_['entry_sort_order'] = 'Порядок сортування';
$_['entry_status'] = 'Статус';
$_['entry_show_option_price'] = 'Показувати ціну біля значень опції';
$_['entry_type'] = 'Тип';
$_['error_name'] = 'Назва опції має містити від 1 до 128 символів!';
$_['error_option_value'] = 'Назва значення опції має містити від 1 до 128 символів!';
$_['error_permission'] = 'Увага: У вас немає дозволу на зміну параметрів!';
$_['error_product'] = 'Увага: Цей параметр не можна видалити, оскільки він наразі призначений %s товарам!';
$_['error_invalid_sort_order'] = 'Невірне значення порядку сортування!';
$_['error_type'] = 'Увага: Потрібні значення опцій!';
$_['heading_title'] = 'Опції';
$_['text_add'] = 'Додати опцію';
$_['text_checkbox'] = 'Прапорець';
$_['text_color'] = 'Колір';
$_['text_choose'] = 'Виберіть';
$_['text_date'] = 'Дата';
$_['text_datetime'] = 'Дата та час';
$_['text_edit'] = 'Редагувати опцію';
$_['text_file'] = 'Файл';
$_['text_image'] = 'Зображення';
$_['text_input'] = 'Вхід';
$_['text_list'] = 'Список опцій';
$_['text_option'] = 'Опція';
$_['text_radio'] = 'Радіо';
$_['text_select'] = 'Виберіть';
$_['text_success'] = 'Успіх: Ви змінили параметри!';
$_['text_text'] = 'Текст';
$_['text_textarea'] = 'Текстове поле';
$_['text_time'] = 'Час';
$_['text_value'] = 'Значення опцій';
// Sidebar
$_['text_settings_card']     = 'Параметри';
$_['text_status_card']       = 'Статус';

// Subtitle

$_['text_list_subtitle'] = 'Керуйте опціями та значеннями товарів';

$_['text_add_option_subtitle'] = 'Визначити нову опцію товару';

$_['text_edit_option_subtitle'] = 'Редагувати значення та налаштування опції';

// Empty state
$_['text_empty_title']       = 'Поки немає опцій';
$_['text_empty_description'] = 'Опції дозволяють обирати варіанти — розмір, матеріал тощо. Додайте першу опцію.';
