<?php

$_['entry_fee'] = 'Плата за рахунок-фактуру';
$_['entry_sort_order'] = 'Порядок сортування';
$_['entry_status'] = 'Статус';
$_['entry_tax_class'] = 'Податковий клас';
$_['entry_total'] = 'Загальна сума замовлення';
$_['error_permission'] = 'Увага: Ви не маєте дозволу змінювати загальну суму оплати Klarna!';
$_['heading_title'] = 'Кларна Фі';
$_['text_denmark'] = 'Данія';
$_['text_edit'] = 'Редагувати загальну плату Klarna';
$_['text_extension'] = 'Розширення';
$_['text_finland'] = 'Фінляндія';
$_['text_germany'] = 'Німеччина';
$_['text_netherlands'] = 'Нідерланди';
$_['text_norway'] = 'Норвегія';
$_['text_success'] = 'Успіх: Ви змінили загальну плату Klarna!';
$_['text_sweden'] = 'Швеція';
$_['text_total'] = 'Враховувати в замовленні';
// Subtitle

$_['text_edit_subtitle'] = 'Налаштуйте параметри та опції розширення';


