<?php

$_['column_action'] = 'Дія';
$_['column_customer'] = 'Ім\'я клієнта';
$_['column_customer_group'] = 'Група клієнтів';
$_['column_email'] = 'Електронна пошта';
$_['column_orders'] = 'Кількість замовлень';
$_['column_products'] = 'Товари';
$_['column_status'] = 'Статус';
$_['column_total'] = 'Всього';
$_['entry_customer'] = 'Клієнт';
$_['entry_date_end'] = 'Дата закінчення';
$_['entry_date_start'] = 'Дата початку';
$_['entry_order_status'] = 'Статус замовлення';
$_['entry_sort_order'] = 'Порядок сортування';
$_['entry_status'] = 'Статус';
$_['error_permission'] = 'Увага: Ви не маєте дозволу на зміну звіту про замовлення клієнтів!';
$_['heading_title'] = 'Звіт про замовлення клієнтів';
$_['text_all_status'] = 'Усі статуси';
$_['text_edit'] = 'Редагувати звіт про замовлення клієнтів';
$_['text_extension'] = 'Доповнення';
$_['text_filter'] = 'Фільтр';
$_['text_list'] = 'Список замовлень покупців';
$_['text_success'] = 'Успіх: Ви змінили звіт про замовлення клієнтів!';
// Subtitle

$_['text_edit_subtitle'] = 'Налаштуйте параметри та опції доповнення';


