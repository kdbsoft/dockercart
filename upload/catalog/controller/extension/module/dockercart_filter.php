...
class ControllerExtensionModuleDockercartFilter extends Controller
{
    ...
    private function sortOptionValuesByOrder(&$options)
    {
        ...
        foreach ($options as $opt_id => $values) {
            ...
            foreach ($query->rows as $row) {
                $sort_orders[(int) $row["option_value_id"]] =
                    (int) $row["sort_order"];
            }
        ...
        foreach ($attributes as $attr_id => $values) {
            ...
            usort($attributes[$attr_id], function ($a, $b) {
                ...
                if (is_numeric($a_lower) && is_numeric($b_lower)) {
                    if (
                        strpos($a_lower, ".") !== false ||
                        strpos($b_lower, ".") !== false
                    ) {
                ...
                if (
                    preg_match("/^(\d+(?:\.\d+)?)/", $a_lower, $matches_a) &&
    ...
    private function sortManufacturersByOrder(&$manufacturers)
    {
        ...
        $sort_orders = [];
        foreach ($query->rows as $row) {
            $sort_orders[(int) $row["manufacturer_id"]] =
                (int) $row["sort_order"];
        }
    ...
    private function encodeFilters(
        $price_min,
        $price_max,
        $manufacturers,
        $attributes,
    ...
    {
        $this->logger->debug(
            "INDEX: Module called with setting=" . json_encode($setting),
        );

        $status = !empty($setting["status"])
            ? $setting["status"]
            : $this->config->get("module_dockercart_filter_status");
        $cache_time = !empty($setting["cache_time"])
            ? $setting["cache_time"]
            : $this->config->get("module_dockercart_filter_cache_time");

        $this->logger->debug(
            "INDEX: status=" . $status . ", cache_time=" . $cache_time,
        );
        ...
        $data["text_active_filters"] = $this->language->get(
            "text_active_filters",
        );
        $data["text_remove_filter"] = $this->language->get(
            "text_remove_filter",
        );
        ...
        $this->logger->debug(
            "CONTROLLER: dcf param = " . ($this->request->get["dcf"] ?? "none"),
        );
        $this->logger->debug(
            "CONTROLLER: Filter currency from URL = " .
                ($filter_currency ?? "none"),
        );
        $this->logger->debug(
            "CONTROLLER: Current session currency = " .
                $this->session->data["currency"],
        );
        $this->logger->debug(
            "CONTROLLER: Parsed filter_manufacturer = " .
                json_encode($filter_manufacturer),
        );
        $this->logger->debug(
            "CONTROLLER: Parsed filter_attribute = " .
                json_encode($filter_attribute),
        );
        $this->logger->debug(
        ...
        if (!empty($filter_manufacturer)) {
            $filter_data["filter_manufacturers"] = array_map(
                "intval",
                $filter_manufacturer,
            );
        }
        ...
        if ($filter_price_min !== "") {
            $price_currency =
                $filter_currency ?: $this->session->data["currency"];
            $filter_price_min = (float) $this->currency->convert(
                $filter_price_min,
                $price_currency,
            ...
            $this->logger->debug(
                "CONTROLLER: Converted price_min from " .
                    $price_currency .
                    " to base: " .
                    $filter_price_min,
            );
        ...
        if ($filter_price_max !== "") {
            $price_currency =
                $filter_currency ?: $this->session->data["currency"];
            $filter_price_max = (float) $this->currency->convert(
                $filter_price_max,
                $price_currency,
                $this->config->get("config_currency"),
            );
            ...
            $this->logger->debug(
                "CONTROLLER: Converted price_max from " .
                    $price_currency .
                    " to base: " .
                    $filter_price_max,
            );
        ...
        $current_currency = isset($this->session->data["currency"])
            ? $this->session->data["currency"]
            : $base_currency;
        ...
        if (!empty($disabledAttributes)) {
            $data["attributes"] = array_filter($data["attributes"], function (
                $attr,
        ...
        if (!empty($disabledOptions)) {
            $data["options"] = array_filter($data["options"], function (
                $opt,
            ) use ($disabledOptions) {
        ...
        $this->logger->debug(
            "CONTROLLER: Model returned attributes = " .
                json_encode(
        ...
        $this->logger->debug(
            "CONTROLLER: Model returned options = " .
                json_encode(
                    array_map(function ($o) {
                        return [
                            "option_id" => $o["option_id"],
                            "name" => $o["name"],
                            "values_count" => count($o["values"]),
                        ];
                    }, $data["options"]),
                ),
        );
        ...
        if ($this->config->get("module_dockercart_filter_seo_mode")) {
            foreach ($data["manufacturers"] as &$manufacturer) {
                $manufacturer["filter_url"] = $this->buildFilterItemUrl(
                    "manufacturer",
            ...
            foreach ($data["attributes"] as &$attribute) {
                foreach ($attribute["values"] as &$value) {
                    $value["filter_url"] = $this->buildFilterItemUrl(
                        "attribute",
        ...
        if (!empty($filter_manufacturer)) {
            $available_mfr_ids = array_column(
                $data["manufacturers"],
                "manufacturer_id",
            );
            $missing_mfr_ids = [];
            ...
            if (!empty($missing_mfr_ids)) {
                ...
                foreach ($missing_mfr_ids as $mfr_id) {
                    if (isset($mfr_data_map[$mfr_id])) {
                        ...
                        if (
                            $this->config->get(
                                "module_dockercart_filter_seo_mode",
                            )
                        ) {
                            $unavailable_mfr[
                                "filter_url"
                            ] = $this->buildFilterItemUrl(
                                "manufacturer",
                                $mfr_id,
                            );
                        }
        ...
        if (!empty($filter_attribute)) {
            $available_attr_ids = array_column(
                $data["attributes"],
                "attribute_id",
            );
            $missing_attr_ids = [];
            ...
            foreach (
                $filter_attribute
                as $selected_attr_id => $selected_values
            ) {
            ...
            if (!empty($missing_attr_ids)) {
                ...
                foreach ($missing_attr_ids as $attr_id) {
                    if (
                        isset($attr_data_map[$attr_id]) &&
                        isset($filter_attribute[$attr_id])
                    ) {
                        $unavailable_attr = [
        ...
        if (!empty($filter_option)) {
            ...
            if (!empty($all_option_ids)) {
                $query_values = $this->db->query(
                    "SELECT option_id, option_value_id, name FROM " .
                        DB_PREFIX .
                        "option_value_description WHERE option_id IN (" .
                        implode(",", array_map("intval", $all_option_ids)) .
                        ") AND language_id = " .
                        (int) $this->config->get("config_language_id"),
                );
                foreach ($query_values->rows as $row) {
                    $value_names_map[$row["option_id"]][
                        $row["option_value_id"]
                    ] = $row["name"];
                }
            }
            ...
            foreach ($filter_option as $selected_opt_id => $selected_values) {
                ...
                foreach ($data["options"] as &$opt) {
                    if ($opt["option_id"] == $selected_opt_id) {
                        ...
                        $existing_value_ids = array_column(
                            $opt["values"],
                            "option_value_id",
                        );
                        foreach ($selected_values as $sel_value_id) {
                            if (!in_array($sel_value_id, $existing_value_ids)) {
                                $value_name = isset(
                                    $value_names_map[$selected_opt_id][
                                        $sel_value_id
                                    ],
                                )
                                    ? $value_names_map[$selected_opt_id][
                ...
                if (!$found_opt) {
                    $query = $this->db->query(
                        "SELECT option_id, name FROM " .
                            DB_PREFIX .
                            "option_description WHERE option_id = " .
                            (int) $selected_opt_id .
                            " AND language_id = " .
                            (int) $this->config->get("config_language_id"),
                    );
                    if ($query->row) {
                        ...
                        foreach ($selected_values as $value_id) {
                            $value_name = isset(
                                $value_names_map[$selected_opt_id][$value_id],
                            )
                                ? $value_names_map[$selected_opt_id][$value_id]
                                : "";
                            if (
        ...
        $has_content =
            !empty($data["manufacturers"]) ||
            !empty($data["attributes"]) ||
            !empty($data["options"]) ||
            ($data["price_range"]["min"] > 0 ||
                $data["price_range"]["max"] > 0);
        ...
        $this->logger->debug(
            "CONTROLLER: Price range ready (with currency conversion from model): min=" .
        ...
        $this->logger->debug(
            "CONTROLLER: Passing to template - filter_attribute = " .
                json_encode($data["filter_attribute"]),
        );
        $this->logger->debug(
            "CONTROLLER: Passing to template - attributes count = " .
        ...
        $data["filter_depth"] = $this->countFilterDepth(
            $filter_manufacturer,
            $filter_attribute,
            $filter_option,
            $user_set_price_min,
        ...
        $data["dynamic_heading"] = $this->generateDynamicHeading(
            $category_id,
            $filter_manufacturer,
            $filter_attribute,
            $filter_option,
        ...
        $data["filter_mode"] =
            $this->config->get("module_dockercart_filter_mode") ?: "button";
        $data["seo_mode"] =
            $this->config->get("module_dockercart_filter_seo_mode") ?: 1;
        $data["items_limit"] =
            (int) $this->config->get("module_dockercart_filter_items_limit") ?:
            10;
        $data["debug_mode"] =
            (int) $this->config->get("module_dockercart_filter_debug") ?: 0;
        $data["filter_theme"] =
            $this->config->get("module_dockercart_filter_theme") ?: "light";
        $data["custom_css"] =
            $this->config->get("module_dockercart_filter_custom_css") ?: "";
        $data["mobile_breakpoint"] =
            (int) $this->config->get(
                "module_dockercart_filter_mobile_breakpoint",
            ) ?:
            768;

        $primary_color =
            $this->config->get("module_dockercart_filter_primary_color") ?:
            "#007bff";
        ...
        $this->document->addStyle(
            "catalog/view/javascript/dockercart_filter/dockercart_filter.css",
        );
        $this->document->addScript(
            "catalog/view/javascript/dockercart_filter/dockercart_filter.js",
        );
        ...
        try {
            return $this->load->view(
                "extension/module/dockercart_filter",
                $data,
            );
        } catch (Exception $e) {
            $this->logger->debug(
                "ERROR in dockercart_filter view: " . $e->getMessage(),
            );
    ...
    public function ajaxFilter()
    {
        ...
        if ($this->request->server["REQUEST_METHOD"] == "POST") {
            ...
            $category_id = isset($post_data["category_id"])
                ? (int) $post_data["category_id"]
                : 0;
            $price_min = isset($post_data["price_min"])
                ? $post_data["price_min"]
                : "";
            $price_max = isset($post_data["price_max"])
                ? $post_data["price_max"]
                : "";
            $manufacturer = isset($post_data["manufacturer"])
                ? $post_data["manufacturer"]
                : [];
            $attribute = isset($post_data["attribute"])
                ? $post_data["attribute"]
                : [];
            ...
            $results = $this->model_extension_module_dockercart_filter->getFilteredProducts(
                $filter_data,
            );
            $total = $this->model_extension_module_dockercart_filter->getTotalFilteredProducts(
                $filter_data,
            );
    ...
    {
        $price_min = !empty($filter_data["price_min"])
            ? $filter_data["price_min"]
            : "";
        $price_max = !empty($filter_data["price_max"])
            ? $filter_data["price_max"]
            : "";
        $manufacturers = !empty($filter_data["manufacturer"])
            ? (array) $filter_data["manufacturer"]
            : [];
        $attributes = !empty($filter_data["attribute"])
            ? $filter_data["attribute"]
            : [];
        $options = !empty($filter_data["option"]) ? $filter_data["option"] : [];

        $dcf = $this->encodeFilters(
            $price_min,
            $price_max,
            $manufacturers,
            $attributes,
    ...
    private function buildActiveFilters(
        ...
        $manufacturer_data = is_array($manufacturer_data)
            ? $manufacturer_data
            : [];
        ...
        $this->logger->debug(
            "buildActiveFilters: Manufacturers input=" .
                json_encode($manufacturers),
        );
        $this->logger->debug(
            "buildActiveFilters: Attributes input=" . json_encode($attributes),
        );
        $this->logger->debug(
            "buildActiveFilters: Options input=" . json_encode($options),
        );
        ...
        if ($price_min !== "" || $price_max !== "") {
            ...
            $this->logger->debug(
                "buildActiveFilters: display_currency=" .
                    $display_currency .
                    ", symbol_left=" .
                    ($currency_symbol_left ?: "empty") .
                    ", symbol_right=" .
                    ($currency_symbol_right ?: "empty"),
            );

            $format_price = function ($price) use (
            ...
            if ($price_min !== "" && $price_max !== "") {
                ...
                $label .=
                    $this->language->get("text_from") .
                    " " .
                    $format_price($price_min);
            } else {
                $label .=
                    $this->language->get("text_to") .
                    " " .
                    $format_price($price_max);
            }
                ...
                if ($manufacturer_name) {
                    $active[] = [
                        ...
                        "label" =>
                            ...
                            ": " .
                            $manufacturer_name,
                        "remove_url" => $this->buildRemoveUrl(
                            $current_url,
                            "manufacturer",
                            $man_id,
                        ),
        ...
        if (!empty($options)) {
            foreach ($options as $opt_id => $values) {
                ...
                foreach ($option_data as $o) {
                    if ($o["option_id"] == $opt_id) {
                        ...
                        foreach ($o["values"] as $ov) {
                            $option_values_map[$ov["option_value_id"]] =
                                $ov["name"];
                        }
                        break;
                    }
                ...
                if (empty($option_values_map)) {
                    $query = $this->db->query(
                        "SELECT option_value_id, name FROM " .
                            DB_PREFIX .
                    ...
                    foreach ($query->rows as $row) {
                        $option_values_map[$row["option_value_id"]] =
                            $row["name"];
                    }
                }

                foreach ($values as $value_id) {
                    $value_name = isset($option_values_map[$value_id])
                        ? $option_values_map[$value_id]
                        : "";
                    if ($option_name && $value_name) {
                        ...
                        $this->logger->debug(
                            "buildActiveFilters: Creating active filter - opt_id=" .
                                $opt_id .
                                ", value_id=" .
                                $value_id .
                                ", label=" .
        ...
        $this->logger->debug(
            "buildActiveFilters: Total active filters = " . count($active),
        );
    ...
    private function buildRemoveUrl(
        $current_url,
        $type,
        $id = null,
        $value = null,
        ...
        $this->logger->debug(
            "buildRemoveUrl: BEFORE - decoded=" . json_encode($decoded),
        );
        ...
        switch ($type) {
            ...
            case "manufacturer":
                $manufacturers = array_values(
                    array_filter($manufacturers, function ($m) use ($id) {
                        return (int) $m != (int) $id;
                    }),
                );
                break;
            ...
            case "attribute":
                if (isset($attributes[$id])) {
                    $attributes[$id] = array_values(
                        array_filter($attributes[$id], function ($v) use (
                            $value,
                        ) {
            ...
            case "option":
                if (isset($options[$id])) {
                    $options[$id] = array_values(
                        array_filter($options[$id], function ($v) use ($value) {
        ...
        $this->logger->debug(
            "buildRemoveUrl: AFTER - manufacturers=" .
                json_encode($manufacturers) .
                ", attributes=" .
    ...
    private function countFilterDepth(
        $manufacturers,
        $attributes,
        $options,
        $price_min,
        ...
        $manufacturer_count = is_array($manufacturers)
            ? count($manufacturers)
            : 0;
    ...
    private function generateDynamicHeading(
        $category_id,
        $manufacturers,
        $attributes,
        $options,
                    ...
                    if ($a["attribute_id"] == $attr_id) {
                        ...
                        usort($sorted_values, function ($x, $y) {
                            ...
                            if (is_numeric($x_lower) && is_numeric($y_lower)) {
                                if (
                                    strpos($x_lower, ".") !== false ||
                                    strpos($y_lower, ".") !== false
                                ) {
                ...
                foreach ($option_data as $o) {
                    if ($o["option_id"] == $opt_id) {
                        ...
                        if (count($value_ids) > 1) {
                            ...
                            usort($value_ids, function ($a, $b) use (
                                $sort_orders,
                            ) {
                                $a_id = (int) $a;
                                $b_id = (int) $b;
    ...
    public function ensureCanonicalUrl()
    {
        ...
        if (
            ...
            strpos($this->request->get["route"], "product/category") === false
        ) {
            $this->logger->debug(
                "ensureCanonicalUrl: Not a category page, skipping",
            );
            return;
        }
        ...
        if (!$canonical_url) {
            $this->logger->debug(
                "ensureCanonicalUrl: Could not build canonical URL",
            );
            return;
        }
        ...
        $current_url = $this->getCurrentUrl();
        $this->logger->debug(
            "ensureCanonicalUrl: Current URL: " . $current_url,
        );
        $this->logger->debug(
            "ensureCanonicalUrl: Canonical URL: " . $canonical_url,
        );
        ...
        if ($current_path !== $canonical_path) {
            $this->logger->debug(
                "ensureCanonicalUrl: URL NOT canonical, redirecting",
            );
    ...
    public function buildCanonicalFilterUrl()
    {
        ...
        if (
            isset($this->request->get["attribute"]) &&
            is_array($this->request->get["attribute"])
        ) {
            $params["attribute"] = $this->request->get["attribute"];

            $this->load->model("extension/module/dockercart_filter");
            $attributes_data = $this->model_extension_module_dockercart_filter->getAttributes(
                ["filter_category_id" => $category_id],
            );
        ...
        if (
            isset($this->request->get["option"]) &&
            is_array($this->request->get["option"])
        ) {
        ...
        if (!empty($params["option"])) {
            $this->load->model("extension/module/dockercart_filter");
            $options_data = $this->model_extension_module_dockercart_filter->getOptions(
                ["filter_category_id" => $category_id],
            );
    ...
    public function buildFilterItemUrl($type, $id, $value = null)
    {
        ...
        $this->logger->debug(
            "buildFilterItemUrl: Current dcf=" .
                ($this->request->get["dcf"] ?? "none"),
        );
        $this->logger->debug(
            "buildFilterItemUrl: Decoded - m=" .
                json_encode($manufacturers) .
                ", a=" .
                json_encode($attributes) .
                ", o=" .
        ...
        $this->logger->debug(
            "buildFilterItemUrl: After toggle - m=" .
                json_encode($manufacturers) .
                ", a=" .
                json_encode($attributes) .
                ", o=" .
    ...
    private function normalizeFilterValueOrder(&$params)
    {
        ...
        if (!empty($params["o"])) {
            ...
            foreach ($opt_pairs as $pair) {
                if (strpos($pair, "_") !== false) {
                    [$opt_id, $val_id] = explode("_", $pair, 2);
                    $opt_id = (int) $opt_id;
                    $val_id = (int) $val_id;
    ...
    private function buildSeoUrl($route, $params = [])
    {
        ...
        $this->logger->debug(
            "buildSeoUrl: SEO mode=" . ($seo_mode ? "ENABLED" : "DISABLED"),
        );
            ...
            if ($category_id) {
                ...
                if ($category_info) {
                    if ($seo_mode) {
                        ...
                        if ($query->num_rows) {
                            // Build full breadcrumb path for all category levels
                            $breadcrumb_keywords = [];
                            $all_found = true;
                            foreach ($path_parts as $pid) {
                                $kw_query = $this->db->query(
                                    "SELECT keyword FROM " .
                                        DB_PREFIX .
                                        "seo_url
                                     WHERE query = 'category_id=" .
                                ...
                                if ($kw_query->num_rows) {
                                    $breadcrumb_keywords[] =
                                        $kw_query->row["keyword"];
                                } else {
                                    $all_found = false;
                                    break;
                                }
                            ...
                            if ($all_found) {
                                $keyword = implode("/", $breadcrumb_keywords);
                            } else {
                                $keyword = $query->row["keyword"];
                            }

                            $base_url = $this->url->link(
                                "product/category",
                                "",
                            );
                            ...
                            $this->logger->debug(
                                "buildSeoUrl: Using SEO URL keyword: " . $url,
                            );
                        } else {
                            $url = $this->url->link(
                                "product/category",
                            ...
                            $this->logger->debug(
                                "buildSeoUrl: No SEO keyword found, using url->link: " .
                                    $url,
                            );
                        }
                    } else {
                        ...
                        $this->logger->debug(
                            "buildSeoUrl: SEO disabled, using url->link with path: " .
                                $url,
                        );
                    }
                } else {
    ...
    public function updateFilters()
    {
        ...
        $category_id = isset($this->request->get["category_id"])
            ...
            : 0;
        $path = isset($this->request->get["path"])
        ...
        if (isset($this->request->get["manufacturer"])) {
            $manufacturers = array_map(
                "intval",
                explode(",", $this->request->get["manufacturer"]),
            );
        ...
        if (isset($this->request->get["price_min"])) {
            ...
            if ($price_min > 0) {
                $price_min = (float) $this->currency->convert(
                    $price_min,
                    $this->session->data["currency"],
                    $this->config->get("config_currency"),
                );
        ...
        if (isset($this->request->get["price_max"])) {
            ...
            if ($price_max > 0) {
                $price_max = (float) $this->currency->convert(
                    $price_max,
                    $this->session->data["currency"],
                    $this->config->get("config_currency"),
                );
        ...
        if (
            ...
            if ($base_currency !== $current_currency) {
                $json["price_range"]["min"] = (float) $this->currency->convert(
                    $json["price_range"]["min"],
                    $base_currency,
                    $current_currency,
                );
                $json["price_range"]["max"] = (float) $this->currency->convert(
                    $json["price_range"]["max"],
                    $base_currency,
                    $current_currency,
                );
            }
