...
class ModelCheckoutOrder extends Model {
	private function resolveOrderTaxNumber($data, $order_id = 0) {
		if (isset($data['tax_number'])) {
			return (string)$data['tax_number'];
		}

		if (!empty($data['customer_id'])) {
			$query = $this->db->query("SELECT tax_number FROM `" . DB_PREFIX . "customer` WHERE customer_id = '" . (int)$data['customer_id'] . "'");

			if ($query->num_rows) {
				return (string)$query->row['tax_number'];
			}
		}

		if ($order_id) {
			$query = $this->db->query("SELECT tax_number FROM `" . DB_PREFIX . "order` WHERE order_id = '" . (int)$order_id . "'");
	...
	public function addOrder($data) {
		...
		try {
			...
			// the unique key serializes concurrent requesters (the loser blocks
			// here until the winner's transaction commits, then hits the
			// duplicate-key path and returns the winner's order).
			if ($session_id !== '') {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_claim SET session_id = '" . $this->db->escape($session_id) . "', order_id = '0', date_added = NOW() ON DUPLICATE KEY UPDATE session_id = session_id");

				$claim_query = $this->db->query("SELECT order_id FROM " . DB_PREFIX . "order_claim WHERE session_id = '" . $this->db->escape($session_id) . "'");

				if ($claim_query->num_rows && (int)$claim_query->row['order_id'] > 0) {
					// Only reuse a claim that still points at a fresh order; a
					// stale claim (abandoned checkout) must not block a new one.
					$existing_query = $this->db->query("SELECT order_id FROM `" . DB_PREFIX . "order` WHERE order_id = '" . (int)$claim_query->row['order_id'] . "' AND date_added > DATE_SUB(NOW(), INTERVAL 15 MINUTE)");

					if ($existing_query->num_rows) {
						$this->db->query("ROLLBACK");

						return (int)$claim_query->row['order_id'];
					}
				}
			}

			$tax_number = $this->resolveOrderTaxNumber($data);

			// Traffic attribution: copy the session's recorded traffic source
			// (UTM / click-id / referrer) onto the order for analytics reporting.
			$utm_source = '';
			$utm_medium = '';
			$utm_campaign = '';

			if ($session_id !== '') {
				$traffic_query = $this->db->query("SELECT `source`, `medium`, `campaign` FROM `" . DB_PREFIX . "dockercart_traffic_source` WHERE `session_id` = '" . $this->db->escape($session_id) . "' ORDER BY `id` DESC LIMIT 1");

				if ($traffic_query->num_rows) {
					$utm_source = $traffic_query->row['source'];
					$utm_medium = $traffic_query->row['medium'];
					$utm_campaign = $traffic_query->row['campaign'];
				}
			}

		$this->db->query("INSERT INTO `" . DB_PREFIX . "order` SET invoice_prefix = '" . $this->db->escape($data['invoice_prefix']) . "', store_id = '" . (int)$data['store_id'] . "', store_name = '" . $this->db->escape($data['store_name']) . "', store_url = '" . $this->db->escape($data['store_url']) . "', customer_id = '" . (int)$data['customer_id'] . "', customer_group_id = '" . (int)$data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', tax_number = '" . $this->db->escape($tax_number) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : '') . "', payment_firstname = '" . $this->db->escape($data['payment_firstname']) . "', payment_lastname = '" . $this->db->escape($data['payment_lastname']) . "', payment_company = '" . $this->db->escape($data['payment_company']) . "', payment_address_1 = '" . $this->db->escape($data['payment_address_1']) . "', payment_address_2 = '" . $this->db->escape($data['payment_address_2']) . "', payment_city = '" . $this->db->escape($data['payment_city']) . "', payment_postcode = '" . $this->db->escape($data['payment_postcode']) . "', payment_country = '" . $this->db->escape($data['payment_country']) . "', payment_country_id = '" . (int)$data['payment_country_id'] . "', payment_zone = '" . $this->db->escape($data['payment_zone']) . "', payment_zone_id = '" . (int)$data['payment_zone_id'] . "', payment_address_format = '" . $this->db->escape($data['payment_address_format']) . "', payment_custom_field = '" . $this->db->escape(isset($data['payment_custom_field']) ? json_encode($data['payment_custom_field']) : '') . "', payment_method = '" . $this->db->escape($data['payment_method']) . "', payment_code = '" . $this->db->escape($data['payment_code']) . "', shipping_firstname = '" . $this->db->escape($data['shipping_firstname']) . "', shipping_lastname = '" . $this->db->escape($data['shipping_lastname']) . "', shipping_company = '" . $this->db->escape($data['shipping_company']) . "', shipping_address_1 = '" . $this->db->escape($data['shipping_address_1']) . "', shipping_address_2 = '" . $this->db->escape($data['shipping_address_2']) . "', shipping_city = '" . $this->db->escape($data['shipping_city']) . "', shipping_postcode = '" . $this->db->escape($data['shipping_postcode']) . "', shipping_country = '" . $this->db->escape($data['shipping_country']) . "', shipping_country_id = '" . (int)$data['shipping_country_id'] . "', shipping_zone = '" . $this->db->escape($data['shipping_zone']) . "', shipping_zone_id = '" . (int)$data['shipping_zone_id'] . "', shipping_address_format = '" . $this->db->escape($data['shipping_address_format']) . "', shipping_custom_field = '" . $this->db->escape(isset($data['shipping_custom_field']) ? json_encode($data['shipping_custom_field']) : '') . "', shipping_method = '" . $this->db->escape($data['shipping_method']) . "', shipping_code = '" . $this->db->escape($data['shipping_code']) . "', utm_source = '" . $this->db->escape($utm_source) . "', utm_medium = '" . $this->db->escape($utm_medium) . "', utm_campaign = '" . $this->db->escape($utm_campaign) . "', session_id = '" . $this->db->escape($session_id) . "', comment = '" . $this->db->escape($data['comment']) . "', total = '" . (float)$data['total'] . "', affiliate_id = '" . (int)$data['affiliate_id'] . "', commission = '" . (float)$data['commission'] . "', marketing_id = '" . (int)$data['marketing_id'] . "', tracking = '" . $this->db->escape($data['tracking']) . "', language_id = '" . (int)$data['language_id'] . "', currency_id = '" . (int)$data['currency_id'] . "', currency_code = '" . $this->db->escape($data['currency_code']) . "', currency_value = '" . (float)$data['currency_value'] . "', ip = '" . $this->db->escape($data['ip']) . "', forwarded_ip = '" .  $this->db->escape($data['forwarded_ip']) . "', user_agent = '" . $this->db->escape($data['user_agent']) . "', accept_language = '" . $this->db->escape($data['accept_language']) . "', date_added = NOW(), date_modified = NOW()");
		...
		if (isset($data['totals'])) {
			...
			// Publish the order id in the claim so concurrent double-submits
			// reuse it instead of inserting a duplicate order.
			if ($session_id !== '') {
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false, $record_history = true, $comment_key = '', $comment_params = array()) {
		...
		try {
			$order_lock = $this->db->query("SELECT order_id FROM `" . DB_PREFIX . "order` WHERE order_id = '" . (int)$order_id . "' FOR UPDATE");

			if (!$order_lock->num_rows) {
				$this->db->query("ROLLBACK");
				return;
			}
