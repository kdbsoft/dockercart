<?php

$_['heading_title'] = 'Личный кабинет';
$_['text_account'] = 'Счет';
$_['text_register'] = 'Зарегистрироваться';
$_['text_guest_heading'] = 'Создайте аккаунт, чтобы начать';
$_['text_address'] = 'Мои адреса';
$_['text_affiliate_add'] = 'Зарегистрируйте партнерский аккаунт';
$_['text_affiliate_edit'] = 'Отредактируйте информацию о партнерской программе';
$_['text_credit_card'] = 'Управление сохраненными кредитными картами';
$_['text_download'] = 'Загрузки';
$_['text_edit'] = 'Редактировать';
$_['text_my_account'] = 'Личный кабинет';
$_['text_my_affiliate'] = 'Партнерский аккаунт';
$_['text_my_newsletter'] = 'Подписка';
$_['text_my_orders'] = 'Мои заказы';
$_['text_newsletter'] = 'Подписка';
$_['text_order'] = 'История заказов';
$_['text_viewed'] = 'Просмотренные товары';
$_['text_password'] = 'Сменить пароль';
$_['text_return'] = 'Запросы на возврат';
$_['text_reward'] = 'Бонусные баллы';
$_['text_tracking'] = 'Пользовательский код отслеживания партнерских программ';
$_['text_transaction'] = 'Транзакции';
$_['text_transactions'] = 'Транзакции';
$_['text_wishlist'] = 'Список желаний';

// Dashboard
$_['text_welcome'] = 'Привет, %s!';
$_['text_total_orders'] = 'Всего заказов';
$_['text_total_spent'] = 'Сумма покупок';
$_['text_total_wishlist'] = 'В избранном';
$_['text_total_reward'] = 'Бонусные баллы';
$_['text_recent_orders'] = 'Последние заказы';
$_['text_no_orders'] = 'У вас пока нет заказов.';
$_['text_go_to_catalog'] = 'Перейти в каталог';
$_['text_quick_actions'] = 'Быстрые действия';
$_['text_edit_profile'] = 'Редактировать профиль';
$_['text_change_password'] = 'Сменить пароль';
$_['text_my_addresses'] = 'Мои адреса';
$_['text_order_number'] = 'Заказ №%s';
$_['text_from'] = 'от';
$_['text_product'] = 'товар';
$_['text_products'] = 'товаров';
