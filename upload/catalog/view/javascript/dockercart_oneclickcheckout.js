...
(function () {
  ...
  function loadCaptcha() {
    ...
    fetch(
      "index.php?route=extension/captcha/" + encodeURIComponent(captchaCode),
      {
        method: "GET",
  ...
  function insertSuccessAlert(message) {
    try {
      ...
      alert.className =
        "alert alert-success alert-dismissible dc-oneclick-success";
      ...
      icon.innerHTML =
        '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#10b981" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:middle;margin-right:8px"><circle cx="12" cy="12" r="10"></circle><path d="M9 12l2 2 4-4"></path></svg>';
  ...
  function showSuccessPopup(message, buttonLabel) {
    try {
      ...
      popup.className =
        "fixed left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-md bg-white text-black border-2 border-green-500 rounded-lg shadow-lg p-6 text-center relative";
      ...
      bigIcon.innerHTML =
        '<svg xmlns="http://www.w3.org/2000/svg" width="56" height="56" viewBox="0 0 24 24" fill="none" stroke="#10b981" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10" fill="rgba(16,185,129,0.08)" stroke="#10b981"></circle><path d="M9 12l2 2 4-4" stroke="#10b981"></path></svg>';
      ...
      closeBtn.innerHTML =
        '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#10b981" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>';
  ...
  function applyPhoneMaskFromCountry() {
    ...
    if (!phoneFormat) {
      var modal = getModal();
      if (modal) {
        phoneFormat = modal.getAttribute("data-default-phone-format") || "";
      }
    }
  ...
  function handleSubmit() {
    ...
    if (telephoneInput && telephoneInput.value) {
      if (
        telephoneInput._phonemask_format &&
        typeof DockercartPhoneMask !== "undefined"
      ) {
        if (
          !DockercartPhoneMask.validate(
        ...
        var phoneValidation = validateAndNormalizeTelephone(
          telephoneInput.value,
        );
    ...
    var originalLabel =
      submitButton.getAttribute("data-original-label") ||
      ...
      .then(function (json) {
        ...
        if (json.success) {
          ...
          try {
            var modal = getModal();
            var successText =
              modal && modal.getAttribute("data-success-text")
                ? modal.getAttribute("data-success-text")
                : json.success;

            // Find all one-click buttons on the page and disable them / change label
            var oneClickButtons = document.querySelectorAll(
              "#button-oneclickcheckout",
            );
            for (var i = 0; i < oneClickButtons.length; i++) {
              try {
                ...
                if (!oneClickButtons[i].getAttribute("data-original-label")) {
                  oneClickButtons[i].setAttribute(
                    "data-original-label",
                    oneClickButtons[i].textContent,
                  );
                }
            ...
            console.error(
              "Error applying success state to one-click button(s):",
              err,
            );
          }
  ...
  document.addEventListener("click", function (e) {
    ...
    var dismissButton = e.target.closest(
      '#oneclickcheckout-modal [data-dismiss="modal"]',
    );
    if (dismissButton) {
      e.preventDefault();
      hideModal();
    ...
    var alertClose = e.target.closest(
  ...
  document.addEventListener("input", function (e) {
    var field = e.target.closest(
      "#oneclickcheckout-form input, #oneclickcheckout-form textarea, #oneclickcheckout-form select",
    );
  ...
  document.addEventListener("change", function (e) {
    ...
    if (e.target && e.target.id === "input-country") {
      ...
      // Fallback to default country format when placeholder is selected
      if (!phoneFormat) {
        var modal = getModal();
        if (modal) {
          phoneFormat = modal.getAttribute("data-default-phone-format") || "";
  ...
  document.addEventListener("focusout", function (e) {
    var telephoneField = e.target.closest(
      '#oneclickcheckout-form input[name="telephone"]',
    );
    if (telephoneField && telephoneField.value) {
      ...
      if (phoneValidation.valid) {
        telephoneField.value = formatTelephoneForDisplay(
          phoneValidation.normalized,
        );
      }
    }

    var emailField = e.target.closest(
      '#oneclickcheckout-form input[name="email"]',
    );
    if (emailField) {
